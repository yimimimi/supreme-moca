...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function updateFasorialTable() {
        const proteccionesRows = proteccionesTable.querySelectorAll("tr");
        const fasorialRows = fasorialTable.querySelectorAll("tr");

        // Iterar por todas las filas de protecciones
        for (let i = 1; i < proteccionesRows.length; i++) {
            const proteccionRow = proteccionesRows[i];
            const fasorialRow = fasorialRows[i];

            // Obtener valores de la tabla de protecciones
            const inValue = parseFloat(proteccionRow.querySelector(".in").value) || 0;
            const fp = parseFloat(proteccionRow.querySelector(".fp").value) || 1;
            
            // Calcular ángulo
            const angulo = Math.acos(fp) * (180 / Math.PI);

            // Actualizar valores en la tabla fasorial
            fasorialRow.querySelector(".in-polar").value = inValue.toFixed(2);
            fasorialRow.querySelector(".angulo").value = angulo.toFixed(2);

            // Calcular valores rectangulares
            const real = inValue * Math.cos((angulo * Math.PI) / 180);
            const imag = -Math.abs(inValue * Math.sin((angulo * Math.PI) / 180));

            fasorialRow.querySelector(".real").value = real.toFixed(2);
            fasorialRow.querySelector(".imag").value = imag.toFixed(2);
        }
    }

    // Agregar event listeners para actualización dinámica
    proteccionesTable.addEventListener("input", (event) => {
        // Verificar si el cambio fue en una fila específica
        const changedRow = event.target.closest("tr");
        const rowIndex = Array.from(proteccionesTable.querySelectorAll("tr")).indexOf(changedRow);

        if (rowIndex > 0) {  // Ignorar la primera fila
            const fasorialRows = fasorialTable.querySelectorAll("tr");
            const fasorialRow = fasorialRows[rowIndex];

            // Obtener valores actualizados
            const inValue = parseFloat(changedRow.querySelector(".in").value) || 0;
            const fp = parseFloat(changedRow.querySelector(".fp").value) || 1;
            
            // Calcular ángulo
            const angulo = Math.acos(fp) * (180 / Math.PI);

            // Actualizar fila correspondiente en tabla fasorial
            fasorialRow.querySelector(".in-polar").value = inValue.toFixed(2);
            fasorialRow.querySelector(".angulo").value = angulo.toFixed(2);
    ...
    // Agregar event listeners para tabla fasorial
    fasorialTable.addEventListener("input", (event) => {
        const changedRow = event.target.closest("tr");
        const rowIndex = Array.from(fasorialTable.querySelectorAll("tr")).indexOf(changedRow);

        if (rowIndex > 0) {  // Ignorar la primera fila
            const proteccionesRows = proteccionesTable.querySelectorAll("tr");
            const proteccionRow = proteccionesRows[rowIndex];

            const inPolar = parseFloat(changedRow.querySelector(".in-polar").value) || 0;
            const angulo = parseFloat(changedRow.querySelector(".angulo").value) || 0;

            // Calcular valores rectangulares
            const real = inPolar * Math.cos((angulo * Math.PI) / 180);
            const imag = -Math.abs(inPolar * Math.sin((angulo * Math.PI) / 180));

            changedRow.querySelector(".real").value = real.toFixed(2);
            changedRow.querySelector(".imag").value = imag.toFixed(2);
        }
    });

    // Inicializar tabla fasorial
    updateFasorialTable();
});